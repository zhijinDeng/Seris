...
系统同时监控两类风险：一类来自设备报警、参数波动或检测异常；另一类来自业务记录本身，例如工单显示关闭却缺少确认原因、结构化措施和失效模式关联。每条风险线程以事件ID关联设备类别、实例、功能、工单、失效模式、点巡检标准、工艺窗口、VIN/批次、处置任务、复机确认和关闭依据。当前仓库实现了带来源片段的关系路径检索、时序检测、闭环完整性审计、Top候选与反证、现场处置关闭和知识债跟踪；生产版GraphRAG将在该机制原型上接入图数据库、向量索引、企业文档切片和查询轨迹服务。
