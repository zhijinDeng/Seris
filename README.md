...
系统同时监控两类风险：一类来自设备报警、参数波动或检测异常；另一类来自业务记录本身，例如工单显示关闭却缺少确认原因、结构化措施和失效模式关联。每条风险线程以事件ID关联设备类别、实例、功能、工单、失效模式、点巡检标准、工艺窗口、VIN/批次、处置任务、复机确认和关闭依据。现场闭环按“异常发现—信息确认—工单发起—任务分派—现场排查—原因判断—维修处置—运行确认—恢复生产—知识沉淀—标准审视—有效性观察”运行，现场处置关闭和知识标准发布是两个不同状态。当前仓库实现了带来源片段的关系路径检索、时序检测、闭环完整性审计、Top候选与反证、现场处置关闭和知识债跟踪；生产版GraphRAG将在该机制原型上接入图数据库、向量索引、企业文档切片和查询轨迹服务。
...
- 企业脱敏数据审计：基于625类设备、9,673个实例、336项功能、1,287条失效模式和406张工单的聚合画像，运行“闭而未解”识别与同类别设备证据迁移。
- 质量知识底座样例：扩展后的设备业务本体、36个图谱节点、32条关系边、历史案例、交互场景、事件准入契约和端到端查询轨迹。
...
- 在线附件区：飞书终稿末尾已上传00–09共10份正式Word材料，并提供互动工作台直达入口与公开源码入口。

## 直接运行

下载仓库后直接打开`app/index.html`。选择工况后先查看由EWMA/CUSUM计算的弱信号和前置量，在反事实实验台选择动作并提交脱敏实测流程，再依次执行飞书派发、质量负责人确认、任务与知识回写验收和关闭校验。关闭动作只读取五项独立证据状态，不会反向制造“通过”结果；关闭后标准变更仍显示为待审批并进入观察期。影子模式会阻断外部写入；L3生产权限不能由浏览器授予。
...
py scripts\test_lifecycle_contract.py
py scripts\run_quality_agent.py
py scripts\run_quality_agent.py --company-audit --json
...
说明：编排脚本即使使用`-DryRun`也会先校验当前授权，用于验证在线对象的参数和权限边界；没有飞书授权时，请只运行上面的本地验证命令。
...
- `docs/attachment-section.xml`：在线终稿末尾的正式材料附件区源片段。
...
演示工艺参数、检测签署身份和时间序列为脱敏仿真数据，不代表赛力斯真实工艺窗口或现网收益。企业数据审计使用赛事提供静态脱敏快照的聚合统计与重键关系，不推断真实发生频率，不把关联候选自动写成根因；原始Word和Excel不进入公共仓库。页面签署与确认只验证流程机制，生产身份、量具、附件、审批实例和不可覆盖时间戳必须由服务端及飞书企业应用回读。系统首期只做决策支持和协同执行，不直接修改PLC/SCADA参数，不替代质量负责人放行，不自动关闭P1事件。公共仓库不提交app secret、access token、refresh token和企业内部人员映射。

GitHub：<https://github.com/zhijinDeng/Seris>

互动工作台：<https://zhijindeng.github.io/Seris/>
