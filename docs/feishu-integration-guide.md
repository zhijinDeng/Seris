...
本机验证环境为Node.js 24、`lark-cli 1.0.85`和飞书事件桥接组件；企业应用已完成机器人身份与用户身份配置，在线质量事件表、任务和复盘文档已完成创建与回验。用户授权具有时效性，任何写入前先运行`lark-cli auth status`，失效时重新登录，不以历史授权状态替代当前检查。
...
本地无授权验证请运行仓库根目录README中的Python契约、企业审计和材料检查命令；这些命令不访问飞书。`orchestrate_feishu_quality_event.ps1`会先执行`auth status --verify`，因此`-DryRun`只表示不写入在线对象，不表示免授权。
...
生产Base采用质量事件、处置任务、复检记录、设备维修、审批记录和复盘知识六表模型；平台回写回执作为审计对象留痕，不单独形成设备台账。企业现有设备数字化平台保留设备类型、实例、工单、故障模式、点检和保养数据的主责；Base不复制主数据所有权。角色权限遵循最小授权：班组只更新接单与处置状态；检测人员签署实测和量具状态；设备工程师确认维修与故障模式；工艺/质量岗位判断质量影响和复机条件；质量负责人确认P1关闭；知识管理员审核标准变更。VIN、供应商批次和质量缺陷按企业数据分级策略脱敏。

## 与设备平台的回写合同

回写采用“草案—审批—幂等写入—回读核对”四步。草案包含源设备编码、源工单号、候选故障模式、证据ID、目标字段、目标版本和有效期；审批必须由设备管理与质量角色共同完成；写入使用事件ID、源对象ID和审批实例ID构造幂等键；回读核对目标版本、修改人、时间和字段哈希。冲突时保留企业平台现值并进入人工合并，不进行末写覆盖。自动允许的只有任务和提醒，故障模式确认、复机放行、标准发布和主平台回写均需授权。
