...
本机已安装Node.js 24、`lark-cli 1.0.81`和飞书事件桥接组件；企业应用`邓植斤的飞书 CLI`已完成机器人身份与用户身份授权。当前用户授权覆盖Base、任务、文档、知识库、消息、审批等业务域，在线质量事件表已完成写入回验。
...
生产Base采用事件、任务、复检、维修、知识五表模型。角色权限遵循最小授权：班组只更新接单与处置状态；设备工程师更新维修字段；质量工程师管理影响范围与关闭依据；质量负责人确认P1关闭；知识管理员维护FMEA和控制计划映射。VIN、供应商批次和质量缺陷按企业数据分级策略脱敏。
