...
生产Base采用知识债/质量事件、诊断任务、现场检查、复机确认、标准变更审批、平台回写回执六表模型。企业现有设备数字化平台保留设备类型、实例、工单、故障模式、点检和保养数据的主责；Base不复制主数据所有权。角色权限遵循最小授权：班组只更新接单与处置状态；检测人员签署实测和量具状态；设备工程师确认维修与故障模式；工艺/质量岗位判断质量影响和复机条件；质量负责人确认P1关闭；知识管理员审核标准变更。VIN、供应商批次和质量缺陷按企业数据分级策略脱敏。
